...
export default function ExecutiveDashboard() {
  ...
  const statCards = [
    { name: 'Platform GMV', value: `Rp ${(kpi.total_gmv || 0).toLocaleString()}`, icon: TrendingUp, color: 'text-emerald-500', bg: 'bg-emerald-500/10' },
    { name: 'Total Orders', value: (kpi.total_orders || 0).toLocaleString(), icon: ShoppingBag, color: 'text-blue-500', bg: 'bg-blue-500/10' },
    { name: 'Active Creators', value: (kpi.active_creators || 0), icon: Users, color: 'text-purple-500', bg: 'bg-purple-500/10' },
    { name: 'Active Campaigns', value: (kpi.active_campaigns || 0), icon: Target, color: 'text-amber-500', bg: 'bg-amber-500/10' },
  ];
          ...
          <Card key={stat.name} className="bg-[#121212] border-white/5 shadow-xl relative overflow-hidden group">
            ...
            <CardContent className="p-6 relative z-10">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center text-xs font-bold text-gray-500 bg-gray-500/10 px-2 py-1 rounded-full">
                  0%
                </div>
              </div>
          ...
          <div className="bg-[#121212] border border-white/5 rounded-[32px] overflow-hidden shadow-xl">
            <div className="p-6 space-y-6">
              <div className="text-center py-8">
                <p className="text-gray-500 font-bold text-sm">No campaigns available.</p>
              </div>
            </div>
