...
export class CmCreatorsService {
  ...
  async onboardCreator(cmId: string, dto: any) {
    ...
    if (existing) throw new BadRequestException('Email sudah terdaftar di sistem.');
    ...
    await this.prisma.$transaction(async (tx) => {
      await tx.user.create({
        data: { id: userId, name: dto.name, email: dto.email, password: hashedPassword, role: 'CREATOR' },
      });
  ...
  async transferCreator(creatorId: string, fromCmId: string, toCmId: string, reason: string) {
    const creator = await this.prisma.creator.findUnique({ where: { user_id: creatorId } });
    if (!creator) throw new NotFoundException('Creator not found');
    const targetCm = await this.prisma.user.findFirst({ where: { id: toCmId, role: 'CM' } });
    if (!targetCm) throw new BadRequestException('Target CM tidak ditemukan');

    await this.prisma.$transaction(async (tx) => {
      await tx.creatorAssignmentLog.create({
        data: { creator_id: creatorId, from_cm_id: creator.cm_id, to_cm_id: toCmId, reason, transferred_by: fromCmId },
      });
      await tx.creator.update({ where: { user_id: creatorId }, data: { cm_id: toCmId } });
      await tx.notification.create({
        data: { user_id: toCmId, title: '🔄 Creator Ditransfer', message: `Creator telah ditransfer kepada Anda oleh CM lain.`, type: 'SYSTEM' },
      });
    });
    return { success: true };
  }
  ...
  async getCMList() {
    ...
    return Promise.all(
      cms.map(async (cm) => ({
        ...cm,
        creatorCount: await this.prisma.creator.count({ where: { cm_id: cm.id } }),
      }))
    );
  }
}
