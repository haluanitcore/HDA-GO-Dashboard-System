...
async function main() {
  ...
  // Alias: original emails still work
  await prisma.user.create({
    data: { name: 'Sarah CM Alt', email: 'sarah@hdago.com', password, role: 'CM' },
  });
  await prisma.user.create({
    data: { name: 'Budi CM Alt', email: 'budi@hdago.com', password, role: 'CM' },
  });
  ...
  for (let i = 0; i < creatorUsers.length; i++) {
    ...
    await prisma.creator.create({ data: { user_id: u.id, creator_level: d.level, gmv_total: d.gmvTotal, gmv_monthly: d.gmvMonthly, total_orders: d.orders, total_campaigns: d.campaigns, total_posts: d.posts, streak_days: d.streak, live_participation: d.live, posting_consistency: d.consistency, cm_id: d.cm, onboarding_status: 'ACTIVE', onboarded_at: new Date() } });
